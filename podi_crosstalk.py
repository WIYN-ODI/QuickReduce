...
def apply_old_crosstalk_correction(hdulist, fpl, extname2id):
    ...
    for row in range(8):
        for column in range(8):
            ...
            xtalk_corr[column] = numpy.zeros(hdulist[1].data.shape, dtype=float)
    ...
    for row in range(8):
        for column in range(8):
            ...
            for xtalk_column in range(8):
                ...
                correction = hdulist[extname2id[xy_name]].data.astype(float) * xtalk_inv[row, xtalk_column, column] #xtalk_matrix[row][xtalk_column][column]
                #if (column != xtalk_column):
                #    saturated = hdulist[extname2id[xy_name]].data >= fpl.crosstalk_saturation_limit(extname)
                #        correction[saturated] = -1 * fpl.crosstalk_saturation_correction(extname)

                xtalk_corr[column] += correction.astype(float) #hdulist[xy_name].data * podi_crosstalk.xtalk_matrix[extname][row][xtalk_column][column]
                    #print xtalk_corr[column][100,100]
